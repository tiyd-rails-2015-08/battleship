class Ship
  ...
  def covers?(x, y) #checks is ship covers a particular coordinate
    @positions.include?([x, y])
  end

  def overlaps_with?(other_ship) #checks if self overlaps with other_ship
  ...
  def fire_at(x, y) #checks if a ship covers a spot, if not yet fired at - allows it to be fired at, adds coordinates to hits array
    if covers?(x, y) && !@hits.include?([x, y])
      @hits << [x, y]
    end
  end

  def sunk?() #checks if length of ship equals length of array
    @length == @hits.length
  end
